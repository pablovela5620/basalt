...
def main(args: Config) -> None:
    golden_t: Int64[ndarray, "n_golden"]
    golden_p: Float64[ndarray, "n_golden 3"]
    candidate_t: Int64[ndarray, "n_candidate"]
    candidate_p: Float64[ndarray, "n_candidate 3"]
    golden_t, golden_p = load(args.golden)
    candidate_t, candidate_p = load(args.candidate)
    count_ratio: float = abs(len(candidate_t) - len(golden_t)) / len(golden_t)

    indices: Int64[ndarray, "n_golden"] = np.searchsorted(candidate_t, golden_t)
    ...
    matched: Bool[ndarray, "n_golden"] = np.abs(candidate_t[indices] - golden_t) <= ASSOCIATION_TOLERANCE_NS
    source: Float64[ndarray, "n_matched 3"] = candidate_p[indices[matched]]
    target: Float64[ndarray, "n_matched 3"] = golden_p[matched]
    if matched.sum() < 10:
    ...
    u: Float64[ndarray, "3 3"]
    vt: Float64[ndarray, "3 3"]
    u, _, vt = np.linalg.svd((target - target_mean).T @ (source - source_mean))
    sign: float = np.sign(np.linalg.det(u @ vt))
    rotation: Float64[ndarray, "3 3"] = u @ np.diag([1.0, 1.0, sign]) @ vt
    aligned: Float64[ndarray, "n_matched 3"] = (rotation @ (source - source_mean).T).T + target_mean
    errors: Float64[ndarray, "n_matched"] = np.linalg.norm(aligned - target, axis=1)
